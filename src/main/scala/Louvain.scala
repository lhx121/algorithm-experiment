...
object Louvain {
    ...
    // Louvain算法的第一步
    def step1(louvainG:Graph[VertexData,Double], m:Double,iter:Int):(Graph[VertexData,Double],Int) = {
        println("==================== step 1 ====================")
        var G = louvainG
        var iterTime = 0
        var canStop = false
        while (iterTime < iter && !canStop) {
    ...
    def execute(sc:SparkContext, initG:Graph[None.type ,Double],iter:Int,Layer:Int):Graph[VertexData,Double]= {
        val writer = new PrintWriter(new File("LouvainCommunity.txt"))
        ...
        var curIter = 0
        var res = step1(louvainG, m.value,iter)
        while (res._2 != 0 && curIter <= Layer) {
            louvainG = res._1
            louvainG = step2(louvainG)
            CommUtil.getCommunities(louvainG,writer)

            res = step1(louvainG, m.value,iter)
            curIter += 1
        }
